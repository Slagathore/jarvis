...
23. [Phase 4: Objects](#23-phase-4-objects) <!-- PARTIAL 2026-05-10: closed-vocab _object_pair_cost done (class match + room continuity + IoU/distance + 15-min staleness). CLIP encoder + open-vocab detector + dedup (§23.3-§23.6) still pending. -->
24. [Phase 5: Interactions](#24-phase-5-interactions) <!-- DONE 2026-05-10: modules/vision/hand_detector.py wraps MediaPipe Hands (sync + detect_async); ObservationBuilder _build_for_frame runs once per frame and attaches per-person hand_bboxes + hand_details metadata; InteractionMonitor (modules/world_model/interactions.py) correlates INTERACTED_WITH × LOST_VISIBILITY → PICKED_UP and INTERACTED_WITH × FIRST_SEEN → PLACED_DOWN with in-flight dedup keyed on object_id; query layer adds what_did_someone_do_with + who_last_touched (registered as LLM tools); orchestrator wires HandDetector + InteractionMonitor at boot, stop() in _shutdown. 4-scenario synthetic test (scripts/test_interactions_synthetic.py) ✅. HANDED_OFF event + dashboard timeline card (§24.6) deferred to a follow-up. -->
